...
from app.agent.state import GenerationState, get_request, get_vision, get_gameplay, get_narrative, _to_serializable
...
## 整合要求
1. 根据所有规范生成一个统一的游戏设计
2. 确定游戏标题（简洁有吸引力，8-20字符，中文）
3. 确定游戏描述（中文）
4. 确定游戏标签（3-5个，中文），标签可以从以下类型中选取或派生：
   - 视觉风格：科幻、像素、卡通、国风、赛博朋克、水墨...
   - 玩法类型：躲避、点击、问答、解谜、跑酷、射击、建造、塔防...
   - 难度/挑战：休闲、高分挑战、硬核、技巧挑战、策略...
   - 情感氛围：紧张刺激、轻松愉快、恐怖、治愈、史诗感...
5. 选择最适合的模板类型作为参考
6. 确认复杂度（必须与 Supervisor 判断一致）

重要：所有输出内容（标题、描述、标签）必须使用中文。
...
{{
    ...
    "complexity": "simple/complex（必须与 Supervisor 判断一致）"
}}
"""
...
async def synthesis_agent(state: GenerationState) -> GenerationState:
    ...
    try:
        ...
        gameplay = get_gameplay(state)
        narrative = get_narrative(state)
        ...
        vision = get_vision(state)
        supervisor_result = state.get("supervisor_result")
        fallback_complexity = supervisor_result.get("complexity", "simple") if supervisor_result else "simple"
