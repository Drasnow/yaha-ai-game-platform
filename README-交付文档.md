...
│  ├── SynthesisAgent  (整合设计)                                 │
│  ├── CodeGeneratorAgent (LLM 代码生成)                         │
│  ├── ValidatorWorkflow (产物验证)                               │
│  ├── RetryWorkflow    (失败重试/自动修复)                        │
...
SynthesisAgent（整合三个专家输出，形成完整游戏设计）
  │
  ▼
CodeGeneratorAgent（LLM 调用生成游戏代码，支持 3 次重试，超时 5 分钟）
  │
  ▼
ValidatorWorkflow（验证生成的 manifest.json 和 index.html 是否合法）
  │
  ├─ [passed] → UploadWorkflow → END
  ├─ [fixable] → RetryWorkflow（最多 3 次自动修复）→ ValidatorWorkflow
  ├─ [unfixable] → CodeGeneratorAgent（重新生成）
  └─ [critical]  → END
```

**重试策略：**
- `CodeGeneratorAgent`：最多 3 次尝试，指数退避（2s → 4s → 8s），超时 5 分钟
- `ValidatorWorkflow` / `UploadWorkflow`：最多 3 次尝试，指数退避（1s → 2s → 4s）
...
| LLM 输出不稳定 | ValidatorWorkflow 验证 + RetryWorkflow 最多 3 次自动修复 |
...
| 失败场景 | 恢复策略 |
|---|---|
| **LLM 输出不稳定 / 生成代码报错** | `ValidatorWorkflow` 验证失败 → `RetryWorkflow` 最多 3 次自动修复（fixable）或标记重新生成（unfixable）；指数退避 2s→4s→8s |
| **构建失败（超时 300s）** | `CodeGeneratorAgent` 超时直接标记任务 FAILED，记录 `errorMessage`；前端展示错误原因 |
...
Agent Service 集成 LangSmith，所有节点（SupervisorAgent、VisionAgent、GameplayAgent、NarrativeAgent、SynthesisAgent、CodeGeneratorAgent、ValidatorWorkflow）执行时自动上报 trace。
...
// GET /api/v1/generation-tasks/:taskId 返回结构
{
  ...
  agentLogs: [
    ...
    { agentName: "CodeGeneratorAgent", step: 5, message: "代码生成完成" },
    { agentName: "ValidatorWorkflow", step: 6, message: "验证通过" },
  ];
}
```
...
| **Agent Service** | LangGraph Multi-Agent（Supervisor → [并行] VisionAgent + GameplayAgent + NarrativeAgent → Synthesis → CodeGeneratorAgent → ValidatorWorkflow → RetryWorkflow → Upload），支持 fixable 自动修复 / unfixable 重新生成 | ✅ 完整 |
...
| **持久化检查点（Redis Saver）** | LangGraph AsyncRedisSaver 持久化检查点，thread_id 绑定 task_id，生成中途崩溃可从断点恢复；30 分钟 TTL 自动清理；自定义 serde 支持所有 Pydantic 状态类型；Redis 不可用时优雅降级 | ✅ 完整 |
...
| **关键 Prompt 示例** | "用 LangGraph 实现游戏生成的 Multi-Agent 状态图，包含 Supervisor / VisionAgent + GameplayAgent + NarrativeAgent（并行）/ Synthesis / CodeGeneratorAgent / ValidatorWorkflow / RetryWorkflow / UploadWorkflow 节点" |
