...
1. 访问 http://localhost:3000 → 查看首页已有示例游戏
2. 点击任意游戏 → Play 页面动态加载 MinIO 远端 iframe ✅
3. 访问 http://localhost:3000/register → 注册账号
4. 登录后访问 http://localhost:3000/create → 输入创意触发 Agent 生成 ✅
...
**验收要求：** 已满足 — "躲避障碍"通过 `/create` 页面触发生成，对应 task_id: `cmqo12wdr000vkgw9m1he0oen`，发布后状态为 PUBLISHED。用户可访问 http://localhost:3000 验证三个游戏均已在首页展示。
...
| 模块 | 状态 | 说明 |
|------|------|------|
| **Play iframe 实际加载** | ⚠️ MinIO bucket public 策略需配置 | MinIO bucket 默认 private，需配置 public read policy 或使用 Presigned URL，否则 iframe 无法加载 |
...
| 优先级 | 功能 | 价值 | 状态 |
| --- | --- | --- | --- |
| P0 | **Play iframe 加载修复** | MinIO bucket public policy + Presigned URL fallback，确保游戏真实可玩 | 待实现 |
...
| 前端构建 | `pnpm build` | ✅ 无错误 |
| 前端开发服务器 | `pnpm dev` | ✅ http://localhost:3000 可访问 |
| Agent Service 启动 | `uv run uvicorn app.main:app --host 127.0.0.1 --port 8000` | ✅ http://localhost:8000/docs 可访问 |
| Play Meta API | `curl http://localhost:3000/api/v1/games/{id}/play-meta` | ✅ 返回 manifestUrl/entryUrl |
